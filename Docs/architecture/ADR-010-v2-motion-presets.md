...
- The initial replaceable `MotionBackend` implementation uses a host-configured trusted native FFmpeg binary to produce a silent video intermediate. The host must configure direct FFmpeg/FFprobe executables; wrappers are unsupported because this POC owns and drains only the direct child process. It writes into a private Replex staging directory and validates expected frame size, fps, duration, probe, decode, artifact size, and SHA-256 before returning a typed `MotionExecutionResult`. The result records artifact ref and SHA-256; width, height, fps, duration, pixel format, alpha mode (`none`), audio presence (`false`), and reported color metadata; backend and preset IDs/versions; source revision/hash; motion job hash; and verification status/evidence. The small JSON verification receipt remains readable after staging cleanup under `.replex-evidence/motion/receipts/<artifact-id>/verification.json`; it binds the source asset ID/hash, target, typed preset parameters, motion job hash, output hash/probe, backend, and FFmpeg version.
...
- A successful internal V2 conversational edit result may include typed `motionExecutions` for the host. These execution summaries and backend details are not returned in model-facing tool results and do not alter service-contract v1.
