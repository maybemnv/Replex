...
**Status:** PR-A through PR-D (Phases 4 and 5) are merged to `main` at `2dc4035`; corrective PR #16 closed the retrospective Gate A event-identity finding at `517b4f4`. Gates A, B, and C have bounded technical evidence on main; Phase 4 selected the native evidence provider (NO-GO for the ffmpeg-skill adapter), and Phase 5 technical validation passed. V2-701 is merged to `main` at `9cfe6cf` with bounded local transport and asynchronous semantic-operation jobs. PR #15 merged as `5950983` with a verified `camera-push.v1` motion-to-composition candidate; Gate D remains open. Three synthetic internal reviews remain pipeline exercise only: 0/3 independent human reviews, with correction times unmeasured. Draft PR #19 proposes V2-702 local import jobs. PR #20 is open with a Phase 9 core-proof candidate whose local mixed-project E2E and seven machine-readable preservation checks pass; it is not merged and does not wire an executor recapture job. Agent, capture/recapture, preview/render job support, and a complete local workflow remain unimplemented on `main`. Live-provider behavior, formal human usefulness, main-branch selective-recapture proof, and production authorization remain unproven.
...
**Candidate status (27 September 2026):** PR #20 implements the bounded core proof and remains open against `main` at `5950983`. The deterministic local mixed-project recapture E2E passes all seven machine-readable preservation checks. `npm run build` passed; the serial full suite passed 44 files / 321 tests with five guarded motion tests skipped, then the motion suite passed all 14 tests separately using FFmpeg/FFprobe 9.0.1. The PR check rollup reports GitGuardian Security Checks success; no GitHub CI test result is claimed. An independent code review found no blockers. This is candidate-branch evidence only; it does not wire recapture into V2-702, close Gate D, declare Gate E complete on `main`, or authorize production.
...
- **Contracts:** One `replace_browser_capture` reducer operation and lineage row for the selected scene; explicit incompatibility result when retained ranges no longer fit. The plan and candidate are detailed in [V2-901 selective recapture design](v2-901-selective-recapture-design.md).
