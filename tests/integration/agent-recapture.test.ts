...
describe.skipIf(!mediaAvailable)("agent edits across selective recapture", () => {
  it("keeps unrelated agent edits and renders the reconciled revision", async () => {
    ...
    try {
      ...
      const agent = runRecordedAgentDraft(base, root, [{ tool: "inspect_project", input: {} }, { tool: "set_title", input: { baseRevisionId: "revision-0", evidenceRefs: ["capture:capture-1"], overlay: { id: "agent-title", sceneId: base.scenes[1].id, kind: "title", text: "Results update", placement: "top", startMs: 0, endMs: 2500 } } }], { requireCompletion: false });
      if (!agent.ok) throw new Error(agent.detail);
      source(join(root, "captures", "0-new.mp4"), 3);
      const replacement = { runId: "run-new", capturedAt: new Date().toISOString(), actionIds: agent.project.scenes[0].actionIds, checkpointActionId: agent.project.scenes[0].checkpointActionId, ...capture(root, 0, "open-demo", "0-new"), changedStepIds: [agent.project.scenes[0].checkpointActionId], reason: "controlled App A change" };
